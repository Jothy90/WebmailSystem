...
<script type="text/javascript">
    ...
    inputBox.onkeyup = function () {
        ...
        <c:forEach items="${inboxMails}" var="mail" varStatus="count">
        if ('<c:out value="${mail.message}"/>'.search(inputBox.value) > 0 || '<c:out value="${mail.sendFrom}"/>'.search(inputBox.value) > 0 || '<c:out value="${mail.subject}"/>'.search(inputBox.value) > 0) {
